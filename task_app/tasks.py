...
def predict_CT(input_file, batch_size=50, gpu=False, verbosity=10):

    data_in = read_file(input_file)
    N_PROC = 1
    ...
    batch_size = min(1000, int(0.2 * file_len))
    ...
    for doc, ctx in cancer_nlp.pipe(list(data_in[['CT_Text_Non_Impressions', 'ID']].to_records(index=False)),
                                        as_tuples=True, batch_size=batch_size, n_process=N_PROC):
        out_ = doc.cats
